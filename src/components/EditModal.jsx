...
const EditModal = () => {
  ...
  return (
    <div className="absolute top-0 left-0 w-full h-full items-center justify-center flex backdrop-blur-[3px] ">
      <form onSubmit={handleSubmit} className="flex flex-col gap-[10px] p-[20px] bg-gray-700 text-white border">
        <input
        value={product?.name}
        ...
        <div className="flex gap-[10px]">
          <button type="button" onClick={() => {
            dispatch(closeModal())
          }} className="p-[4px] bg-amber-700">close</button>
          <button type="submit" className="p-[4px] bg-green-700">change</button>
        </div>
      </form>
    </div>
  );
